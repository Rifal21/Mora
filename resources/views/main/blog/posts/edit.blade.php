...
@section('content')
    <div class="container mx-auto mt-28 px-6 max-w-4xl">
        ...
        <form action="{{ route('blogPosts.update', $blogPost->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ $blogPost->title }}" class="w-full px-4 py-2 border rounded-lg">
            </div>
            ...
            <div>
                ...
                <select name="blog_category_id" class="w-full px-4 py-2 border rounded-lg">
                    @foreach ($categories as $cat)
                        <option value="{{ $cat->id }}" {{ $blogPost->blog_category_id == $cat->id ? 'selected' : '' }}>
                            {{ $cat->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="content" rows="8" class="w-full px-4 py-2 border rounded-lg">{{ $blogPost->content }}</textarea>
            </div>
            ...
            <div>
                ...
                @if ($blogPost->image)
                    <img src="{{ asset('storage/' . $blogPost->image) }}" alt="" class="w-40 mt-3 rounded-lg">
                @endif
            </div>
            ...
            <div class="flex gap-4">
                ...
                <div class="flex-1">
                    ...
                    <input type="text" name="author" value="{{ $blogPost->author }}"
